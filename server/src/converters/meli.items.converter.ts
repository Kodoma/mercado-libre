...
import {
  ...
  MeliSearchResponse, Pictures
} from "../models/meli.models";
...
class MeliItemsConverterImpl implements MeliItemsConverter {
  ...
  private convertCategories(filters: connector.Filter[]): string[] {
    ...
    category && category.values.map( e => {
  ...
  private async convertItem(ItemResult: connector.MeliItemResponse): Promise<Item> {
    let description ;
    description = await this.convertItemDescription(ItemResult.id);
    return {
      id: ItemResult.id,
  ...
  private convertItems(results: connector.MeliItemResponse[]): Item[] {
    ...
    if(!results) return;
     ...
     results.map(r => ItemsResult.push( {
       id: r.id,
       title: r.title,
       price: {
